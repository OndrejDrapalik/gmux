...
TEMPLATE_DIR="${GMUX_TEMPLATE_DIR:-${XDG_CONFIG_HOME:-$HOME/.config}/gmux/templates}"
...
pane_label() {
  ...
  if ((pane_index > 0)); then
    ...
    printf '%s: %s  (%s)' "$((pane_index + 1))" "$command" "$split"
  else
...
case "${1:-pick}" in
  ...
  apply) apply_template "${2:?usage: gmux-template-picker.sh apply TEMPLATE.json ROOT SESSION}" "${3:?usage: gmux-template-picker.sh apply TEMPLATE.json ROOT SESSION}" "${4:?usage: gmux-template-picker.sh apply TEMPLATE.json ROOT SESSION}" ;;
  *) die "usage: gmux-template-picker.sh [pick [ROOT]|preview TEMPLATE.json|apply TEMPLATE.json ROOT SESSION]" ;;
esac
